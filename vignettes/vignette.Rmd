...
`lmms` summarize each feature into a unique time profile.
...
In this plot, we can observe that the highest silhouette coefficient is achieved when `r ncomp = pca.ncomp$choice.ncomp` (4 clusters).
...
It extracts key features on each component to determine a signature.
...
In this plot, we can observe that the highest silhouette coefficient is achieved when `r ncomp = pls.ncomp$choice.ncomp` (4 clusters).
...
In this plot, we can observe that the highest silhouette coefficient is achieved when `r ncomp = block.ncomp$choice.ncomp` (2 clusters).
