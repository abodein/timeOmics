...
To optimize the number of clusters, the number of PCA components needs to be optimized first (`getNcomp`).
...
Thanks to the previous plot, highest silhouette coefficient is achieved when `r ncomp = pca.ncomp$choice.ncomp`(4 clusters).
...
For each feature (molecule), one can see for each feature (molecule) the selected component where the contribution is maximum, the direction of the contribution and the attributed cluster.
...
Thanks to the previous plot, highest silhouette coefficient is achieved when `r ncomp = pls.ncomp$choice.ncomp` (4 clusters).
...
Thanks to the previous plot, highest silhouette coefficient is achieved when `r ncomp = block.ncomp$choice.ncomp` (2 clusters).
